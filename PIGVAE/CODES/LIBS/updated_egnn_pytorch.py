...
#
#
# Here I've added inter-layer normalization and prenorm/postnorm options
#
...
class EGNN_Network(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_global_tokens = 4,
        # ADDITION
        inter_layer_norm_feats: bool = False,
        inter_layer_norm_coors: bool = False,
        inter_layer_norm_coors_scale_init: float = 1e-2,
        inter_layer_prenorm: bool = True,    # normalize before each EGNN layer
        inter_layer_postnorm: bool = False,  # or after each EGNN layer
        **kwargs
    ):
        super().__init__()
        ...
        # ADDED: inter-layer normalization and prenorm/postnorm options
        self.inter_layer_norm_feats = inter_layer_norm_feats
        self.inter_layer_norm_coors = inter_layer_norm_coors
        self.inter_layer_prenorm = inter_layer_prenorm
        self.inter_layer_postnorm = inter_layer_postnorm

        if inter_layer_norm_feats:
            self.inter_feats_norms = nn.ModuleList([nn.LayerNorm(dim) for _ in range(depth)])
        else:
            self.inter_feats_norms = None

        if inter_layer_norm_coors:
            self.inter_coors_norms = nn.ModuleList([CoorsNorm(scale_init = inter_layer_norm_coors_scale_init) for _ in range(depth)])
        else:
            self.inter_coors_norms = None
    ...
    def forward(
        ...
        for ind, (global_attn, egnn) in enumerate(self.layers):
            # NEW: inter-layer pre-norm
            if self.inter_layer_norm_feats and self.inter_layer_prenorm:
                feats = self.inter_feats_norms[ind](feats)
            if self.inter_layer_norm_coors and self.inter_layer_prenorm:
                coors = self.inter_coors_norms[ind](coors)
            ...
            # NEW: inter-layer post-norm
            if self.inter_layer_norm_feats and self.inter_layer_postnorm:
                feats = self.inter_feats_norms[ind](feats)
            if self.inter_layer_norm_coors and self.inter_layer_postnorm:
                coors = self.inter_coors_norms[ind](coors)
