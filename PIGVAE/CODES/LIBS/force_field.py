...
class EnergyCalculator:
    ...
    def lennard_jones_energy(self, coords, cutoff=1.0, switch_start=0.8, exclude_bonded=True):
        """
        Corrected Lennard-Jones energy calculation with switching function
        
        Args:
            coords: Tensor of shape [num_atoms, 3] in nm
            cutoff: Distance cutoff in nm (default 1.0 nm)
            switch_start: Starting point for switching function as fraction of cutoff
        ...
        # Exclude bonded pairs, 1-3 interactions, and scale 1-4 interactions
        if exclude_bonded and hasattr(self, 'bond_indices'):
            # Exclude 1-2 interactions (directly bonded)
            bond_indices = self.bond_indices.to(coords.device)
            ...
            # Exclude 1-3 interactions if topology information available
            if hasattr(self, 'angle_indices'):
                for i in range(self.angle_indices.shape[0]):
                    idx1, _, idx3 = self.angle_indices[i]
                    mask[idx1, idx3] = False
                    mask[idx3, idx1] = False
        ...
        # Use larger minimum distance for numerical stability
        safe_dists = torch.clamp(dists, min=0.1)  # Safer minimum distance
        
        # Calculate LJ potential: E = 4ε[(σ/r)^12 - (σ/r)^6]
        ratio = sigma_ij / safe_dists
        ratio6 = ratio ** 6
        ratio12 = ratio6 ** 2
        E_lj = 4 * eps_ij * (ratio12 - ratio6)
        
        # Apply switching function for smooth cutoff
        switch_start_dist = switch_start * cutoff
        switch_mask = (dists >= switch_start_dist) & (dists <= cutoff)
        
        # Calculate switching function: S = 1 - 10x^3 + 15x^4 - 6x^5
        # where x = (r - rswitch)/(rcut - rswitch)
        x = (dists - switch_start_dist) / (cutoff - switch_start_dist)
        switch = 1.0 - 10.0*(x**3) + 15.0*(x**4) - 6.0*(x**5)
        switch = torch.where(switch_mask, switch, torch.ones_like(dists))
        switch = torch.where(dists > cutoff, torch.zeros_like(dists), switch)
        
        # Apply switching function to energy
        E = E_lj * switch
...
def physics_loss(energy_calculator, pos_pred, batch, use_log =True,
    ...
    for i in range(num_graphs):
        ...
        try:
            ...
            if use_log:
                bond_energy = torch.log1p(bond_energy)
                angle_energy = torch.log1p(angle_energy)
                lj_energy = torch.log1p(lj_energy)
